...
namespace service
{
    ...
    private const string sqlBuscarPrecio= "SELECT A.Id, Codigo, Nombre, A.Descripcion, IdMarca, M.descripcion Marca, IdCategoria, C.descripcion Categoria,ImagenUrl,Precio FROM articulos A, marcas M, categorias C where IdMarca = M.id and IdCategoria = C.id and precio @comparacion";
    private const string sqlBuscarCadena= "SELECT A.Id, Codigo, Nombre, A.Descripcion, IdMarca, M.descripcion Marca, IdCategoria, C.descripcion Categoria,ImagenUrl,Precio FROM articulos A, marcas M, categorias C where IdMarca = M.id and IdCategoria = C.id and @campo LIKE '@cadena'";
